...
echo 7. NO Blanca (Variable)				D. CESTAS ITALIA (no etiqueta cajas)
...
IF %ETIQUETA%==LVH7KG (
	...
	IF %MAQUINA%==LINEAD (
	IF %DEBUG%==ON ( 
			ECHO HAS EEGIT LA ETIQUETA LVH 7 KILOS I LA MAQUINA LINEAD
			PAUSE
			)
	...
	IF %MAQUINA%==TODAS (
	IF %DEBUG%==ON ( 
			ECHO HAS EEGIT LA ETIQUETA LVH 7 KILOS I TOTES les Maquines
			PAUSE
			)
	copy \\192.168.1.10\c\ETIQUETAS\Utilitzades\nectarines_LVH_NO_7kilos_VAR_CAIXA.lab \\192.168.1.10\c\ETIQUETAS\CAIXA_SAMO.LAB
	copy \\192.168.1.10\c\ETIQUETAS\Utilitzades\nectarines_LVH_NO_7kilos_VAR_CAIXA.lab \\192.168.1.10\c\ETIQUETAS\CAIXA_MONOCALIBRE.LAB
